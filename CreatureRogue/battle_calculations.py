...
def perform_move(move, attacking_creature, defending_creature, static_game_data):
    ...
    if move.pp <= 0:
        ...
        if not hit_calculation(move, attacking_creature, defending_creature):
            ...
            if move.move_data.target.identifier in ['selected-pokemon', 'random-opponent', 'all-other-pokemon', 'opponents-field', 'all-opponents', 'entire-field']:
                target = defending_creature
        
            if target:
                if move.move_data.damage_move():
                    new_messages, hp_loss = damage_calculation(move, attacking_creature, target, static_game_data.type_chart)
