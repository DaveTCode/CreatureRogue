...
from CreatureRogue.data_layer.data import ACCURACY_STAT, EVASION_STAT, StaticGameData
...
class Loader:
    ...
    def _load_ailments(conn) -> dict[int, Ailment]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT id, name FROM move_meta_ailments INNER JOIN move_meta_ailment_names ON move_meta_ailments.id = move_meta_ailment_names.move_meta_ailment_id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )
    ...
    def _load_stats(conn) -> dict[int, Stat]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT id, name, short_name FROM stats INNER JOIN stat_names ON stats.id = stat_names.stat_id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )
    ...
    def _load_colors(conn) -> dict[int, Color]:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id, name, red, green, blue FROM pokemon_colors INNER JOIN pokemon_color_names ON pokemon_colors.id = pokemon_color_names.pokemon_color_id WHERE local_language_id="
            + str(settings.LOCAL_LANGUAGE_ID)
        )
    ...
    def _load_pokeballs(conn) -> dict[int, Pokeball]:
        ...
        for (
            ...
            pokeballs[pokeball_id] = Pokeball(
                pokeball_id,
                name,
                catch_rate,
                (r_top, g_top, b_top),
    ...
    def _load_move_targets(conn) -> dict[int, MoveTarget]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT id, identifier, name, description FROM move_targets INNER JOIN move_target_prose ON id = move_target_id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )
    ...
    def _load_types(conn) -> dict[int, Type]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT types.id, name FROM types INNER JOIN type_names ON type_id = types.id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )
    ...
    def _load_moves(
        ...
        types: dict[int, Type],
        stats: dict[int, Stat],
        move_targets: dict[int, MoveTarget],
        ailments: dict[int, Ailment],
    ) -> dict[int, MoveData]:
        ...
        for (
            ...
            stat_cur.execute(
                f"SELECT stat_id, change FROM move_meta_stat_changes WHERE move_id = {move_id}"
            )
    ...
    def _load_species(
        ...
        stats: dict[int, Stat],
        growth_rates: dict[int, GrowthRate],
        moves: dict[int, MoveData],
    ) -> dict[int, Species]:
        logging.info("Loading species")
        species = {}
        cur = conn.cursor()
        cur.execute(
            f"SELECT species_id, creature_id, pokedex_number, name, height, weight, base_experience, color_id, growth_rate_id, flavor_text, genus, capture_rate FROM creature_species_data WHERE pokedex_id = {settings.POKEDEX_ID} AND local_language_id = {settings.LOCAL_LANGUAGE_ID}"
        )
        ...
        for (
            ...
            stats_cur.execute(
                f"SELECT stat_id, base_stat FROM pokemon_stats INNER JOIN stats ON stats.id = pokemon_stats.stat_id WHERE pokemon_id = {creature_id}"
            )
            ...
            moves_cur.execute(
                f"SELECT move_id, level FROM pokemon_moves WHERE pokemon_move_method_id=1 AND pokemon_id={creature_id} AND version_group_id = {settings.VERSION_GROUP_ID}"
            )
            ...
            species[species_id] = Species(
                ...
                species_stats,
                base_exp,
                growth_rates[growth_rate_id],
                name[0:1],
                colors[color_id],  # TODO - What if the creature has no name?
                level_moves,
                flavor_text,
                genus,
                capture_rate,
    ...
    def _load_regions(conn) -> dict[int, Region]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT id, identifier, name FROM regions INNER JOIN region_names ON regions.id = region_names.region_id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )
    ...
    def _load_locations(conn, regions: dict[int, Region]) -> dict[int, Location]:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT id, identifier, name, region_id FROM locations INNER JOIN location_names ON locations.id = location_names.location_id WHERE local_language_id={settings.LOCAL_LANGUAGE_ID} AND NOT region_id IS NULL"
        )
    ...
    @staticmethod
    def _load_location_areas(
        conn, locations: dict[int, Location], species: dict[int, Species]
    ) -> dict[int, LocationArea]:
        logging.info("Loading location areas")
        location_areas = {}
        cur = conn.cursor()
        cur.execute(
            f"SELECT location_areas.id, location_areas.identifier, location_area_prose.name, location_areas.location_id FROM location_areas INNER JOIN location_area_prose ON location_areas.id = location_area_prose.location_area_id WHERE NOT location_areas.location_id IS NULL AND local_language_id={settings.LOCAL_LANGUAGE_ID}"
        )

        for area_id, identifier, name, location_id in cur.fetchall():
            rate_cur = conn.cursor()
            rate_cur.execute(
                f"SELECT encounter_method_id, rate FROM location_area_encounter_rates WHERE version_id = (SELECT MAX(version_id) FROM location_area_encounter_rates WHERE location_area_id = {area_id}) AND location_area_id = {area_id}"
            )

            enc_cur = conn.cursor()
            enc_cur.execute(
                f"SELECT species_id, MIN(min_level), MAX(max_level), MAX(rarity), encounter_method_id FROM encounters INNER JOIN pokemon on pokemon_id = pokemon.id INNER JOIN encounter_slots ON encounter_slots.id = encounters.encounter_slot_id WHERE location_area_id = {area_id} GROUP BY pokemon_id, encounter_method_id"
            )
