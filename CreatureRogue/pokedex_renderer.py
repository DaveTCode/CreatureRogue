...
class PokedexRenderer:
    ...
    def _render_lines(self):
        ...
        """
        libtcod.console.set_default_foreground(self.console, settings.POKEDEX_LINE_COLOR)
        libtcod.console.hline(self.console, 0, PokedexRenderer.header_height, PokedexRenderer.width)
        for i in range(PokedexRenderer.column_width, PokedexRenderer.width, PokedexRenderer.column_width):
            libtcod.console.vline(self.console, i, PokedexRenderer.header_height + 1, PokedexRenderer.column_height)
    ...
    def _render_selection(self, selected_row, selected_column, left_most_column):
        ...
        """
        libtcod.console.set_default_foreground(self.console, settings.POKEDEX_LINE_COLOR)
        libtcod.console.put_char(self.console, (selected_column - left_most_column) * PokedexRenderer.column_width + 19, selected_row + PokedexRenderer.header_height + 1, '<')
    ...
    def _render_species(self, pokedex, left_most_column):
        ...
        for pokedex_number in pokedex:
            ...
            libtcod.console.print(self.console, column * PokedexRenderer.column_width + 1, row + PokedexRenderer.header_height + 1, str(pokedex_number) + ". " + name)
    ...
    def _render_details_box(self, species, status):
        ...
        """
        libtcod.console.set_default_foreground(self.console, settings.POKEDEX_LINE_COLOR)
        libtcod.console.print_frame(self.console, 19, 15, 43, 16)  # TODO: Generalise to widths
        
        libtcod.console.print(self.console, 23, 16, u"No. {0.pokedex_number:0=3d}  {0.name}".format(species))

        if status == 2:
            libtcod.console.print(self.console, 23, 17, u"  {0.genus} Pokemon".format(species))
            libtcod.console.print(self.console, 23, 18, u"Type(s): {0}".format(', '.join(str(t) for t in species.types)))
            libtcod.console.print(self.console, 23, 19, "Height: {0}".format(species.imperial_height_str()))
            libtcod.console.print(self.console, 23, 20, "Weight: {0}".format(species.imperial_weight_str()))
            
            libtcod.console.print_rect(self.console, 20, 22, 41, 14, species.flavor_text)
        elif status == 1:
            libtcod.console.print(self.console, 23, 17, "  ????? Pokemon".format(species))
            libtcod.console.print(self.console, 23, 18, "Type(s): ?????")
            libtcod.console.print(self.console, 23, 19, "Height: ??'??\"")
            libtcod.console.print(self.console, 23, 20, "Weight: ????.? lbs.")
