...
if __name__ == "__main__":
    ...
    print(f"You've selected a: Lv.{args.attacking_creature_level} {attacking_species}")

    defending_species = game.static_game_data.species[args.defending_creature_id]
    print(f"You've selected a: Lv.{args.defending_creature_level} {defending_species}")

    attacking_moves = [
        Move(move_data)
        for move_data in attacking_species.move_data_at_level(args.attacking_creature_level)
    ]

    wild_creature = BattleCreature(
        creature_creator.create_wild_creature(
            game.static_game_data, defending_species, args.defending_creature_level
        ),
        game.static_game_data,
    )

    game_data.battle_data = BattleData(
        game_data,
        BattleCreature(
            Creature(
                attacking_species,
                args.attacking_creature_level,
                None,
                None,
    ...
    game.state = BattleState(
        game,
        game.game_data,
        game.battle_renderer,
        game.level_up_renderer,
        game.catch_graphic_renderer,
    )
    game.game_loop()
